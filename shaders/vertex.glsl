...
void main(){
    ...
    newpos.y-=mod(u_time/5.,3.);
    float progress=smoothstep(-1.,4.,newpos.y);
    float progress1=smoothstep(-1.,8.,newpos.y);
    // newpos.z += cnoise(vec4(position, u_time/2.));
    
    vProgress=progress;
    newpos.z+= +progress*-newpos.y*newpos.y - newpos.x*newpos.x;
    
    vec3 noise1=1.*(position*vec3(1.,3.,1.)+u_time*vec3(0.,1.5,0.))*vec3(5.,1.,1.);
    vec3 noise2=1.*(position+u_time*vec3(0.,0.9,0.))*vec3(5.,1.,1.);


    newpos.x+=0.4*progress*(cnoise(vec4(noise1,0.*u_time)));
    newpos.yz+=0.7*progress1*(cnoise(vec4(noise2,u_time))+0.2);
